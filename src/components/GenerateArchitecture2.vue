...
import { ref } from 'vue'
import Floor from './Floor2.vue';
import { store } from '../store'
import CustomConnectionLine from './CustomConnectionLine.vue';
import {floorToNode, generateEdge} from '../floorUtils.js';
 ...
 */
const elements = ref([
  ...store.netArch.floors.map((f) => {
    return floorToNode(f);
  }),
  ...store.netArch.floors.reduce((connectors, floor) => {
    return [...connectors, ...floor.parents.map(id => {
      return generateEdge(id, floor.id);
    })];
  }, [])
])
...
<style>
@import 'https://cdn.jsdelivr.net/npm/@vue-flow/core@1.24.2/dist/style.css';
@import 'https://cdn.jsdelivr.net/npm/@vue-flow/core@1.24.2/dist/theme-default.css';
...
.architecture-map {
  ...
  width: calc(100vw - var(--drawer-width) - 1rem);
